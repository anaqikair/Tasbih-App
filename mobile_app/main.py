...
def get_data_path():
    # Safe writable directory for mobile and desktop
    if platform.system() == "Windows":
        return os.path.join(os.environ.get("APPDATA", ""), "TasbihApp_data.json")
    return os.path.join(os.path.expanduser("~"), "TasbihApp_data.json")
...
def main(page: ft.Page):
    ...
    def check_goals():
        ...
        if reached_count:
            ...
            if last_streak != today_str: 
                ...
                page.snack_bar = ft.SnackBar(content=ft.Text("🎉 Daily Goal Reached!", color=BG_COLOR, weight=ft.FontWeight.BOLD), bgcolor=PRIMARY_COLOR, duration=3000)
    ...
    home_view = ft.Container(
        content=ft.Column(
            controls=[
                ...
                ft.Row([
                    ft.IconButton(icon=ft.Icons.REMOVE_CIRCLE_OUTLINE, icon_color=TEXT_MUTED, icon_size=32, on_click=reduce_count),
                    ft.Container(width=40),
                    ft.IconButton(icon=ft.Icons.REFRESH_ROUNDED, icon_color=TEXT_MUTED, icon_size=32, on_click=reset_count),
    ...
    def toggle_general_timer(e):
        ...
        if general_timer_left <= 0: return
        ...
        ft.SafeArea(
            content=ft.Column(
                controls=[
                    ...
                    home_view, timer_view, stats_view
                ],
            ), expand=True
        )
    )
